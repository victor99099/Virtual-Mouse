...
const WebSocket = requireFromExecutable('ws');
const QRCode = requireFromExecutable('qrcode');
const robot = requireFromExecutable('robotjs');
const express = requireFromExecutable('express');
const http = requireFromExecutable('http');
const { mouse } = requireFromExecutable('@nut-tree-fork/nut-js');
const { spawn } = requireFromExecutable('child_process');
const app = express();
const server = http.createServer(app);
const wss = new WebSocket.Server({ server });
const path = requireFromExecutable('path');
...
function startDrawing() {
  const dotnetAppPath = path.join(__dirname, 'win-x64', 'MouseDrawerApp.exe');

  drawingProcess = spawn(dotnetAppPath);
...
function changeDrawingColor(drawingProcess, color) {
  if (drawingProcess) {
    drawingProcess.stdin.write(`changecolor ${color}\n`);
...
wss.on('connection', (ws) => {
  ...
  ws.on('message', (message) => {
    ...
    if (command === 'move') {
      ...
      drawingProcess = startDrawing();
      changeDrawingColor(drawingProcess, color)
